...
class TripletDset(torchvision.datasets.MNIST):
    ...
    def _get_data(self):
        pt_train = f"{self.root}/MNIST/processed/training.pt"
        pt_test = f"{self.root}/MNIST/processed/test.pt"
    ...
    def _create_triple(self, tensors, labels):
        ...
        for idx in range(10):
            ...
            trainval_img, test_img, trainval_label, test_label = train_test_split(
                img, label, test_size=self.test_size, random_state=self.random_state
            )
